...
const GROUPS: ShortcutGroup[] = [
  {
    ...
    items: [
      ...
      { keys: ["Ctrl", "Shift", "F"], label: "Глобальний пошук" },
      { keys: ["?"], label: "Ця довідка" },
      { keys: ["Esc"], label: "Закрити модалку" },
    ],
  },
  {
    title: "Робота з рядком",
    items: [
      { keys: ["Ctrl", "B"], label: "Закладка для активного рядка" },
      { keys: ["Ctrl", "S"], label: "Зберегти файл" },
      { keys: ["↑", "↓"], label: "Попередній / наступний рядок (у Monaco)" },
    ],
  },
  {
    title: "Контекстне меню (ПКМ по рядку)",
    items: [
    ...
    <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-6" onClick={onClose}>
      ...
      >
        <div className="px-5 py-3 border-b border-[var(--border-soft)] flex items-center gap-3">
          ...
          <h2 className="text-[14px] font-semibold text-[var(--text-strong)]">Гарячі клавіші</h2>
        ...
        <div className="px-5 py-2.5 border-t border-[var(--border-soft)] flex items-center text-[10.5px] text-[var(--text-faint)]">
          ...
          <span className="ml-auto">
            <button className="dp-btn dp-btn--ghost" onClick={onClose}>Закрити</button>
          </span>
        </div>
      </div>
    </div>
